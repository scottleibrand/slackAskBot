...
def oldmain():
    ...
    for subdir in os.listdir('trimmed/'):
        ...
        for filename in sorted(os.listdir(os.path.join('trimmed/', subdir))):
            ...
            # If the cumulative token count is less than tokens_threshold, combine the messages and continue to read in the next file
            if token_count + file_token_count < tokens_threshold:
                ...
                continue
            elif token_count + file_token_count < maxtokens:
                if token_count < maxtokens:
                    ...
                    # Split the previous messages
                    split_data = splitdata(combined_data, tokens_threshold)
                    if split_data is not None:
                        for i, message in enumerate(split_data):
                            if not os.path.exists(combined_subdir):
                                os.makedirs(combined_subdir)
                            output_filename = start_date + '-' + end_date + '-part' + str(i) + '.json'
                            print(f'Writing out {output_filename} with {get_token_count(message)} tokens')
                            with open(os.path.join(combined_subdir, output_filename), 'w') as f:
                                json.dump(message, f)
                # If the cumulative token count is between tokens_threshold and maxtokens, write out the combined messages first
                if not os.path.exists(combined_subdir):
                    os.makedirs(combined_subdir)
                ...
                if start_date is not None and end_date is not None:
                    ...
                    if combined_data is not None:
                        with open(os.path.join(combined_subdir, output_filename), 'w') as f:
                            json.dump(combined_data, f)
            else: # token_count + file_token_count >= maxtokens
                # If the cumulative token count is greater than maxtokens call a split function
                print(f'{filename} combined token count: {token_count+file_token_count} tokens: splitting')
                split_data = splitdata(data, tokens_threshold)
                #print(split_data)
                # Write out the split messages into multiple files
                for i, message in enumerate(split_data):
                    if not os.path.exists(combined_subdir):
                        os.makedirs(combined_subdir)
                    output_filename = start_date + '-' + end_date + '-part' + str(i) + '.json'
